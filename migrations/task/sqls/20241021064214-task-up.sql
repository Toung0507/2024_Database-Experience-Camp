    ...
    (
        (
            SELECT id
               FROM "USER"
              WHERE name = '王小明'
                AND email = 'wXlTq@hexschooltest.io'
        ),
        (
            SELECT id
              FROM "CREDIT_PACKAGE"
    ...
    (
        (
            SELECT id
               FROM "USER"
              WHERE name = '好野人'
                AND email = 'richman@hexschooltest.io'
        ),
        (
            SELECT id
              FROM "CREDIT_PACKAGE"
        ...
        (
            SELECT "COACH".id
             ...
             INNER JOIN "USER"
                    ...
                    AND "USER".email = 'lee2000@hexschooltest.io'
        ),
        (
            SELECT id
              FROM "SKILL"
...
INSERT INTO
    "COURSE" (
        ...
        start_at ,end_at   ,max_participants,meeting_url
    )
VALUES
    (
        (
...
(
    (
        SELECT u.id
          FROM "USER" u
         WHERE u."name" = '好野人'
           AND u.email = 'richman@hexschooltest.io'
    ),
    (
        SELECT c.id
          FROM "COURSE" c
...
SELECT u."name" AS 會員姓名 ,
       c."name" AS 課程名稱 ,
       cb.status AS 課程狀態,
        c.meeting_url AS 課程連結
...
SELECT cp.user_id AS user_id ,SUM(purchased_credits) AS total
...
SELECT cb.user_id AS user_id ,COUNT(cb.user_id) AS total
  FROM "COURSE_BOOKING" cb
 INNER JOIN "USER" u
            ON u.id = cb.user_id
        AND (u."name" = '王小明'
...
SELECT "COURSE_BOOKING".user_id AS user_id,
  ...
  FROM (
        SELECT cp.user_id AS user_id ,SUM(purchased_credits) AS total_credit
          FROM "CREDIT_PURCHASE" cp
         INNER JOIN "USER" u
                    ON u.id = cp.user_id
                AND (u."name" = '王小明'
 ...
 INNER JOIN (
                SELECT cb.user_id AS user_id,COUNT(cb.user_id) AS used_credit
                  FROM "COURSE_BOOKING" cb
                 INNER JOIN "USER" u
                         ON u.id = cb.user_id
                        AND (u."name" = '王小明'
...
SELECT "SKILL".name AS 專長名稱 ,COUNT(coach_id) AS coach_total
  FROM "COACH_LINK_SKILL"
 INNER JOIN "SKILL" ON "COACH_LINK_SKILL".skill_id = "SKILL".id
  GROUP BY "SKILL".name
...
-- 6-3. 查詢：計算 11 月份組合包方案的銷售數量 (實作時間已經12月，改為12月的銷售數量)
-- 顯示須包含以下欄位： 組合包方案名稱, 銷售數量
SELECT cpac."name" AS  組合包方案名稱 ,COALESCE(COUNT(cpur.credit_package_id), 0) AS 銷售數量
  FROM "CREDIT_PURCHASE" cpur 
  RIGHT JOIN "CREDIT_PACKAGE" cpac 
          ON cpur.credit_package_id = cpac.id
  WHERE cpur.created_at BETWEEN '2024-12-01 00:00:00' AND '2024-12-31 00:00:00'
  GROUP BY cpac."name";

-- 6-4. 查詢：計算 11 月份總營收（使用 purchase_at 欄位統計）(實作時間已經12月，改為12月的銷售數量)
-- 顯示須包含以下欄位： 總營收
SELECT COALESCE(SUM(cpur.price_paid), 0) AS 總營收
  FROM "CREDIT_PURCHASE" cpur 
 WHERE cpur.purchase_at BETWEEN '2024-12-01 00:00:00' AND '2024-12-31 00:00:00';

-- 6-5. 查詢：計算 11 月份有預約課程的會員人數（需使用 Distinct，並用 created_at 和 status 欄位統計）
-- (實作時間已經12月，改為12月的銷售數量)
-- 顯示須包含以下欄位： 預約會員人數
SELECT COUNT(DISTINCT user_id) 
  FROM "COURSE_BOOKING" cb 
 WHERE cb.created_at BETWEEN '2024-12-01 00:00:00' AND '2024-12-31 00:00:00'
   AND cb.status != '課程已取消';